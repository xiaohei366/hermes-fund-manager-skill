...
- 行情、板块和新闻原因必须按 `references/data_validation.md` 做数据验证，并在报告中输出验证状态。
...
- **盘前汇报**：交易日开盘前使用，重点看隔夜消息、政策、资金预期和今日主线，不强行汇报盘中涨跌。
- **盘中汇报**：交易日上午/下午盘中使用，重点看实时市场、板块涨跌 Top5、资金面、情绪面、持仓方向影响和风险提示。
- **收盘后复盘**：交易日收盘后使用，复盘市场、板块、持仓影响、原因、今日主线、调整建议、建议置信度和数据验证状态。报告末尾必须询问用户是否要根据最新持仓截图更新本地持仓信息。
- **非交易日基金日报**：周末或节假日使用，不汇报不存在的今日涨跌，重点输出基金行业日报、政策新闻和持仓结构风险检查。
...
如果用户明确指定盘前、盘中、收盘后或非交易日模式，按指定模式执行。若未指定，按 Asia/Shanghai 当前时间和交易日状态自动推断。
...
收盘后复盘必须在报告末尾询问用户是否更新持仓信息。若用户提供持仓截图或结构化持仓文本，先生成候选快照和 diff，不得直接写入；只有用户明确确认后才调用 `scripts/portfolio_store.py merge-snapshot`。
